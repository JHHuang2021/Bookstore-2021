...
- **指令说明**
  - **检索图书指令**
    ...
    - 以 `[ISBN]` 字典升序依次输出满足要求的图书信息，每个图书信息输出格式为 `[ISBN]\t[Book-Name]\t[Author]\t[Keyword]\t[Price]\t[库存数量]\n`
      ...
      - 无附加参数时，所有图书均满足要求
      - 附加参数内容为空则操作失败
      - `[Keyword]` 中出现复数个关键词则操作失败

  - **购买图书指令**
    - `{1}` `buy [ISBN] [Quantity]`
    - 购买指定数量的指定图书（由于本系统不包括支付系统，故仅需减少库存）
      - 没有符合条件的图书则操作失败
...
- 尝试使用 Github Action 对上传代码自动执行指定操作。在以下两个功能中实现一个得 `<3>`，实现两个得 `<4>`
  - **Task 8.** 进行 Lint 检查
